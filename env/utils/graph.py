...
class Graph:
    def __init__(self, args, job_num, machine_num):
        ...
        # do myself
        ##
        # self.m_op_pair = [[] for _ in range(machine_num)]
        ##
    ...
    def add_job(self, job):
        ...
        for i in range(job.op_num):
            ...
            # do myself
            ## there are some repeated edges
            # for m_id in op.machine_and_processtime:
            #     if len(self.m_op_pair[m_id[0]]) != 0:
            #         self.op_op_edge_src_idx = torch.cat((self.op_op_edge_src_idx,  torch.tensor([[self.op_num for _ in range(len(self.m_op_pair[m_id[0]]))]])), dim=1)
            #         self.op_op_edge_tar_idx = torch.cat((self.op_op_edge_tar_idx,  torch.tensor([[op_id for op_id in self.m_op_pair[m_id[0]]]])), dim=1)
            #         self.op_op_edge_src_idx = torch.cat((self.op_op_edge_src_idx,  torch.tensor([[op_id for op_id in self.m_op_pair[m_id[0]]]])), dim=1)
            #         self.op_op_edge_tar_idx = torch.cat((self.op_op_edge_tar_idx,  torch.tensor([[self.op_num for _ in range(len(self.m_op_pair[m_id[0]]))]])), dim=1)

            #     self.m_op_pair[m_id[0]].append(self.op_num)
            ##
    ...
    def update_feature(self, jobs, machines, current_time):
        ...
        # machine feature
        # [status(2), time to complete, waiting time]
        for m in machines:
            feat = [0] * 2
