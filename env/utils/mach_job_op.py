...
class Machine:
    ...
    def avai_time(self):        # return complete time
...
class Job:
    def __init__(self, args, job_id, op_config):
        ...
        self.acc_expected_process_time = list(itertools.accumulate([op.expected_process_time for op in self.operations[::-1]]))[::-1]
...
class Operation:
    def __init__(self, args, job_id, config):
        ...
        self.node_id = -1
        # if not the first operation, then they are not ready
        if self.op_id == 0:
            self.avai_time = 0
        else:
            self.avai_time = MAX
