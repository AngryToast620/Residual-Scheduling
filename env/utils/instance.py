...
class JSP_Instance:
    ...
    def current_avai_ops(self):
        ...
        # buffer_times = np.full(self.machine_num, np.inf)
        ...
        for m in self.machines:
            ...
            buffer_time = np.inf
            # find the buffer_time for this machine first
            for job in self.jobs:
            ...
            #         if m.machine_id == machine_and_processtime[0] and buffer_time > machine_and_processtime[1]:
            #             buffer_time = machine_and_processtime[1]

            # # using buffer_time to decide the available operations
            # for job in self.jobs:
            #     if job.done() == True or job.current_op().avai_time > self.current_time + buffer_time:
            #         continue
            ...
            # buffer_times[m.machine_id] = buffer_time
